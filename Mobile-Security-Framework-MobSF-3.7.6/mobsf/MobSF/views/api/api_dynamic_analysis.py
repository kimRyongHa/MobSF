...
from mobsf.DynamicAnalyzer.views.android.operations import perform_touch
...
def api_touch(request):
    """POST - Perform a touch action on the emulator."""
    try:
        # Parse x and y coordinates from request
        data = request.POST
        x_coord = data.get('x')
        y_coord = data.get('y')
        ...
        if not x_coord or not y_coord:
            return make_api_response({'error': 'Missing x or y coordinate'}, 422)
        
        # Convert coordinates to integer
        x = int(x_coord)
        y = int(y_coord)
        
        # Call the function to perform touch operation
        success = perform_touch(x, y)
        
        # Check if the operation was successful
        if success:
            return make_api_response({'status': 'Success'}, 200)
        else:
            return make_api_response({'error': 'Failed to perform touch'}, 500)
            
    except Exception as e:
        # Handle unexpected errors
        return make_api_response({'error': 'Server error', 'details': str(e)}, 500)
